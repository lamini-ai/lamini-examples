...
def ask_model_question(channel_id, model, question):
    ...
    if model == "rag":
        rag_examples = get_rag_examples(question)
        return ask_lamini_question(channel_id, rag_model, question, rag_examples)

    return ask_lamini_question(channel_id, model, question, training_examples)

def get_rag_examples(question):
    index = RagRunner("/app/lamini-rag-slackbot/models")

    examples = index.get_examples(question)

    return training_examples + examples

def ask_lamini_question(channel_id, model, question, examples):
    token = config["channel_token_mappings"][channel_id]["token"]

    system_prompt = "You are a medical coding expert who has read the ICD11 standard from WHO. You are asked to code the following medical report. If you don't know the answer, reply that you don't know."
    ...
    task_parameters = "Healthcare provider: Mayo Clinic Rochester.\nPatient Name: Eleanor Williams.\nYear: 2023."

    prompt = f"<s>[INST] {system_prompt}\n\n{task_parameters}\n\nAnswer the following question.\n\n{question} [/INST]"

    prompt = add_examples(prompt, system_prompt, examples)
...
def add_examples(prompt, system_prompt, examples):
    complete_prompt = ""
